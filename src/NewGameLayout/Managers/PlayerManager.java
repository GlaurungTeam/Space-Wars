package NewGameLayout.Managers;

/**
 * Created by ivan on 2/27/17.
 */
public class PlayerManager {
    //TODO:Player class must only have fields and getters&setters all other methods must be managed by the PlayerManager class!

    //Initialize Player entity with spawn coordinates(row 129-134 in GameController)
    //updatePlayerLocation() row 91 in Player class
    //must implement all methods from Player class
}
